...
pub const Error = error{
    cgltf_error_data_too_short,
    cgltf_error_unknown_format,
    cgltf_error_invalid_json,
    cgltf_error_invalid_gltf,
    cgltf_error_invalid_options,
    cgltf_error_file_not_found,
    cgltf_error_io_error,
    cgltf_error_out_of_memory,
    cgltf_error_legacy_gltf,
};

fn resultToError(result: cgltf_result) Error {
    return switch (result) {
        cgltf_result_data_too_short => .cgltf_error_data_too_short,
        cgltf_result_unknown_format => .cgltf_error_unknown_format,
        cgltf_result_invalid_json => .cgltf_error_invalid_json,
        cgltf_result_invalid_gltf => .cgltf_error_invalid_gltf,
        cgltf_result_invalid_options => .cgltf_error_invalid_options,
        cgltf_result_file_not_found => .cgltf_error_file_not_found,
        cgltf_result_io_error => .cgltf_error_io_error,
        cgltf_result_out_of_memory => .cgltf_error_out_of_memory,
        cgltf_result_legacy_gltf => .cgltf_error_legacy_gltf,
        else => {
...
pub fn parse(data: []const u8, options: ?cgltf_options) Error!*cgltf_data {
    const parse_option = options orelse std.mem.zeroes(cgltf_options);
    const out: *cgltf_data = undefined;
    const result = cgltf_parse(&parse_option, data.ptr, data.len, &out);
...
pub fn parseFile(path: [:0]const u8, options: ?cgltf_options) Error!*cgltf_data {
    const parse_option = options orelse std.mem.zeroes(cgltf_options);
    const out: *cgltf_data = undefined;
    const result = cgltf_parse_file(&parse_option, path.ptr, &out);
    if (result == cgltf_result_success) {
        return out;
    }
    return resultToError(result);
...
pub fn free(data: *cgltf_data) void {
    cgltf_free(data);
}
