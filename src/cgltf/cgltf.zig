...
pub fn appendMeshPrimitive(
    ...
    // Indices.
    {
        ...
        if (accessor.*.stride == 1) {
            assert(accessor.*.component_type == api.cgltf_component_type_r_8u);
            ...
            assert(accessor.*.component_type == api.cgltf_component_type_r_16u);
            ...
            assert(accessor.*.component_type == api.cgltf_component_type_r_32u);
    ...
    // Attributes.
    {
        ...
        while (attrib_index < num_attribs) : (attrib_index += 1) {
            ...
            if (attrib.*.type == api.cgltf_attribute_type_position) {
                assert(accessor.*.type == api.cgltf_type_vec3);
                assert(accessor.*.component_type == api.cgltf_component_type_r_32f);
                @memcpy(
                    @ptrCast([*]u8, &positions.items[positions.items.len - num_vertices]),
                    data_addr,
                    accessor.*.count * accessor.*.stride,
                );
            } else if (attrib.*.type == api.cgltf_attribute_type_normal and normals != null) {
                assert(accessor.*.type == api.cgltf_type_vec3);
                assert(accessor.*.component_type == api.cgltf_component_type_r_32f);
                @memcpy(
            ...
            } else if (attrib.*.type == api.cgltf_attribute_type_texcoord and texcoords0 != null) {
                assert(accessor.*.type == api.cgltf_type_vec2);
                assert(accessor.*.component_type == api.cgltf_component_type_r_32f);
                @memcpy(
            ...
            } else if (attrib.*.type == api.cgltf_attribute_type_tangent and tangents != null) {
                assert(accessor.*.type == api.cgltf_type_vec4);
                assert(accessor.*.component_type == api.cgltf_component_type_r_32f);
                @memcpy(
