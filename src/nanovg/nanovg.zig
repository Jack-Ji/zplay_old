...
pub const GlyphPosition = api.NVGglyphPosition;
...
pub fn beginFrame(window_width: f32, window_height: f32, device_pixel_ratio: f32) void {
    api.nvgBeginFrame(ctx, window_width, window_height, device_pixel_ratio);
}

// Cancels drawing the current frame.
pub fn cancelFrame() void {
    api.nvgCancelFrame(ctx);
}
...
pub fn lineCap(cap: LineCap) void {
    const c_cap: c_int = switch (cap) {
        .butt => api.NVG_BUTT,
        .round => api.NVG_ROUND,
        .square => api.NVG_SQUARE,
    };
    api.nvgLineCap(ctx, c_cap);
}
...
//   [b d f]
//   [0 0 1]
// void nvgTransform(NVGcontext* ctx, float a, float b, float api, float d, float e, float f);
...
pub fn createImageRgba(w: u32, h: u32, flags: ImageFlags, data: []const u8) Image {
    return Image{ .handle = api.nvgCreateImageRGBA(ctx, @intCast(c_int, w), @intCast(c_int, h), @bitCast(u6, flags), data.ptr) };
}

// Updates image data specified by image handle.
pub fn updateImage(image: Image, data: []const u8) void {
    api.nvgUpdateImage(ctx, image.handle, data.ptr);
}
...
pub fn deleteImage(image: Image) void {
    api.nvgDeleteImage(ctx, image.handle);
}
...
pub fn linearGradient(sx: f32, sy: f32, ex: f32, ey: f32, icol: Color, ocol: Color) Paint {
    return api.nvgLinearGradient(ctx, sx, sy, ex, ey, icol, ocol);
}
...
pub fn pathWinding(dir: Winding) void {
    api.nvgPathWinding(ctx, @enumToInt(dir));
}
...
pub fn arc(cx: f32, cy: f32, r: f32, a0: f32, a1: f32, dir: Winding) void {
    api.nvgArc(ctx, cx, cy, r, a0, a1, @enumToInt(dir));
}

// Creates new rectangle shaped sub-path.
pub fn rect(x: f32, y: f32, w: f32, h: f32) void {
    api.nvgRect(ctx, x, y, w, h);
}
...
pub fn ellipse(cx: f32, cy: f32, rx: f32, ry: f32) void {
    api.nvgEllipse(ctx, cx, cy, rx, ry);
}

// Creates new circle shaped sub-path.
pub fn circle(cx: f32, cy: f32, r: f32) void {
    api.nvgCircle(ctx, cx, cy, r);
}
...
pub fn textGlyphPositions(x: f32, y: f32, string: []const u8, positions: []GlyphPosition) void {
    ...
    _ = api.nvgTextGlyphPositions(ctx, x, y, std.meta.assumeSentinel(string, 0), string.ptr + string.len, positions.ptr, @intCast(c_int, positions.len));
}
