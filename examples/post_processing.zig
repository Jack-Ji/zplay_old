...
var kernel_shapen = post_processing.Convolution.Kernel.initShapen();
var kernel_blur = post_processing.Convolution.Kernel.initBlur();
var kernel_edge = post_processing.Convolution.Kernel.initEdgeDetection();
...
fn loop(ctx: *zp.Context) void {
    ...
    Framebuffer.use(null);
    {
        ...
        switch (pp_selection) {
            .gamma_correction => {
                pp_gamma_correction.draw(&ctx.graphics, fb_material, gamma_value);
            },
            .grayscale => {
                pp_grayscale.draw(&ctx.graphics, fb_material);
            },
            .inversion => {
                pp_inversion.draw(&ctx.graphics, fb_material);
            },
            .convolution => {
                pp_convolution.draw(
                    &ctx.graphics,
    ...
    dig.beginFrame();
    {
        ...
        if (dig.begin(
            ...
            _ = dig.combo_Str(
                ...
                "gamma correction\x00grayscale\x00inversion\x00convolution\x00",
                null,
            );
